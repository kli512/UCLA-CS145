...
def getBeta_Newton(train_x, train_y, num_iter, verbose):
    ...
    for iter in range(0, num_iter):
        ...
        y_pred = sigmoid(train_x @ beta)
        grad = train_x.T @ (train_y - sigmoid(train_x @ beta))
    
        neg_hessian = train_x.T @ np.diag(y_pred * (1 - y_pred)) @ train_x
        beta += np.linalg.inv(neg_hessian) @ grad
