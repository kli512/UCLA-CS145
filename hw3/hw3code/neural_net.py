...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        def softmax_loss(x, y):
            ...
            n = y.shape[0]
            y_onehot = np.zeros((n, y.max() + 1))
            y_onehot[np.arange(n), y] = 1

            exps = np.exp(x - x.max(axis=-1)[:, None])
            softmax = exps / exps.sum(axis=-1)[:, None]
            log_likelihoods = -np.log(np.sum(softmax * y_onehot, axis=-1))

            loss = np.mean(log_likelihoods)
            dx = (softmax - y_onehot) / n
