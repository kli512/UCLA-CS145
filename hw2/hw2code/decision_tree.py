...
def compute_info_gain(data, att_name, y_name):
    ...
    info_gain -= sum(map(lambda x: x[1] / total_counts * compute_entropy(data.loc[data[att_name] == x[0]][y_name]), zip(vals, counts)))
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
...
def comput_gain_ratio(data, att_name, y_name):
    ...
    # STRART YOUR CODE HERE  #
    #========================#
    att_info -= sum(map(lambda count: count/total_counts * np.log2(count/total_counts), counts))
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
...
class DecisionTree(object):
    ...
    def make_tree(self, train_data, parent_node_class = None):
        ...
        if len(np.unique(data[self.y_name])) <= 1:
            ...
            # STRART YOUR CODE HERE  #
            #========================#
            if len(np.unique(data[self.y_name])) > 0:
